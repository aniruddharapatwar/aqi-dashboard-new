...
UPDATED FOR: LSTM Regression Models
"""
...
class Config:
    ...
    MODEL_PATH = BASE_DIR / "Classification_trained_models"  # Update if needed
...
class DataManager:
    ...
    def get_location_data(self, location_name):
        ...
        # Return current (last row) and historical (up to 240 rows for 24h model)
...
class ResponseValidator:
    ...
    def validate_response(response: str, context: Dict) -> Dict:
        ...
        disclaimer_patterns = [
            r'\n*\s*⚠ ï¸\s*\*\*Important:\*\*.*?medical advice.*?(?:\n\n|\Z)',
...
async def root():
    return {
        "message": "AQI Dashboard API - LSTM Version",
        "version": "2.1.0",
        "status": "running",
